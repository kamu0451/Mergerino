...
HighlightingPage::HighlightingPage()
{
    ...
    auto layout = layoutCreator.emplace<QVBoxLayout>().withoutMargin();
    {
        ...
        auto customSound = layout.emplace<QHBoxLayout>().withoutMargin();
        {
            auto label = customSound.append(this->createLabel<QString>(
                [](const auto &value) {
                    if (value.isEmpty())
                    {
                        return QString("Default sound: Mergerino Ping");
                    }
        ...
        layout.append(createCheckBox(
            "Play highlight sound even when Mergerino is focused",
            getSettings()->highlightAlwaysPlaySound));
        layout.append(createCheckBox(
            "Flash taskbar only stops highlighting when Mergerino is focused",
