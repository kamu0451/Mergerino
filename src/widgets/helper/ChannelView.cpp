...
void ChannelView::drawMessages(QPainter &painter, const QRect &area)
{
    ...
    auto initialY = -static_cast<int>(messagesSnapshot[start]->getHeight() *
    ...
    for (; ctx.messageIndex < messagesSnapshot.size(); ++ctx.messageIndex)
    {
        ...
        const auto animationProgress = this->messageArrivalAnimationProgress(
        ...
        if (const auto animatedY = this->messageLayoutShiftAnimationY(
                messagesSnapshot[ctx.messageIndex]))
        {
            renderY = *animatedY;
        }
            ...
            if (paintResult.hasAnimatedElements)
            {
                ...
                else
                {
                    animationArea.setBottom(
                        (animatedCtx.y + layout->getHeight()));
                                  ...
                                  MessageLayout *layout)
{
    ...
    switch (link.type)
    {
        ...
        break;
        case Link::Reconnect: {
            this->underlyingChannel_.get()->userReconnect();
        }
        break;
