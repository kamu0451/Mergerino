...
- Bugfix: "Copy Stream Overlay URL" now pins the tab itself, not its position. The copied URL used to embed the tab's index, so reordering, adding, or removing tabs silently pointed the OBS overlay at a different tab's chat. Every tab now carries a permanent id kept in the saved window layout; the copied URL uses it and follows the tab through reorders, renames, and app restarts. Previously copied numeric URLs still work but stay positional -- re-copy the URL once to upgrade.
- Bugfix: Live-status polling can no longer silently die about an hour into a session. YouTube's servers recycle HTTP/2 connections hourly, and a poll request caught on the closing connection could finish with no response, no error, and no timeout -- the request watchdog only started once a request was actually on the wire, so these ghost requests waited forever and the tab's poll loop stopped for good. A YouTube tab then never noticed the channel going live until the app was restarted. The watchdog now arms as soon as the request is created (restarting once it is sent), aborts never-sent requests with a "timed out, never sent" warning in the diagnostic log, and the poll loop recovers on its own.
- Bugfix: A YouTube tab now joins the moment the channel goes live, even right after a waiting room. A pre-live waiting room put the upcoming video id in a "recently failed" cooldown (5-60 min), and the tab then discarded the channel probes' confirmed-live results for that id until the cooldown expired -- Twitch/Kick joined instantly while YouTube looked dead, and only restarting the app or re-adding the source escaped early. Confirmed-live probe results are now trusted immediately.
- Minor: Every launch now writes a diagnostic log to %APPDATA%\Mergerino\Logs\mergerino-diag.log (32 MiB cap, one rollover), with the provider chat engines at debug verbosity. "A platform didn't pick the stream up" reports are now diagnosable after the fact without relaunching with special flags.
- Bugfix: A YouTube tab now shows every chat message. Polling had been stuck on YouTube's default "Top chat" view, which only surfaces a curated subset of messages -- on a smaller stream that view stays (near) empty, so a genuinely-live channel delivered nothing and looked broken. Polling now switches to the unfiltered "Live chat" view on the first response.
...
- Bugfix: Kick chat history no longer shuffles messages sent within the same second, the correct subscriber badge tier is shown, channel emotes survive a transient Cloudflare error, and quiet channels stop re-fetching history on every tab open.
- Bugfix: "Hide emote-only messages" no longer hides paid cheer messages; toggling "Show deletions of single messages" now applies to messages already on screen; the "Automatically close user popup" setting works again (default off, matching previous behavior); the tab-folder context menu says "Delete folder" instead of "Delete tab".
- Bugfix: Windows toast notifications keep emoji in stream titles intact instead of showing broken characters, the header viewer count is readable in the light theme, and the released zip now includes the Kick onboarding images (the login walkthrough rendered blanks).
- Minor: Removed constant background timers in every chat input box and locked prediction dialogs, and made the per-user logging check allocation-free on the message path. A refactor (commit b909cd9) dropped the `.execute()` on the shared 7TV fetch helper, so every 7TV request (Twitch/Kick channel, global, and personal emote sets) was built and silently discarded -- with no error message -- leaving only whatever was in the on-disk cache. Restored the `NetworkRequest` debug assert that previously caught a forgotten `.execute()`.
...
- Minor: Added a "Float emotes on the chat background" setting that drifts emotes from emote-only messages across the chat background instead of cluttering the message list. Pairs with "Hide emote-only messages". Choose the motion ("Bounce" or "Fly-by") with the "Background emote animation" dropdown, and set how long each emote lingers (1-600 seconds) with "Background emote animation duration" -- for Fly-by this also sets the arc travel time, so a higher value drifts emotes across more slowly.
- Merge: Pulled in the upstream Fixlation features from 1.2.6 through 1.3.2 while keeping this fork's work: interactive Twitch polls & predictions, YouTube OAuth sign-in + chatting/moderation, command autocomplete, tab folders, chat identity/mode indicators, Kick level/subscriber badges, viewer-list Mod/VIP categories, per-user logging, StreamDatabase badge feed, post-update patch-notes dialog, pin-application, relative-time activity tabs, bulk tab select, and the Ctrl+D platform-swap hotkey.
- Bugfix: YouTube and TikTok messages on merged tabs now trigger highlights, highlight sound, window alerts, and the /mentions channel again (the merge had left the highlight helpers uncalled).
- Security: Stopped the YouTube OAuth token from ever being written to `--log-file`, and sanitized per-user log paths (a crafted chat display name could otherwise escape the log directory).
- ToS/Security: Reworked Twitch polls, predictions, moderation, and the StreamDatabase badge lookup to use only the official Helix API. The upstream implementations drove Twitch's private `gql.twitch.tv` endpoints by impersonating the first-party web client, replaying a browser integrity token, scraping the user's web-session cookie, and minting a token via the Android-TV app's client id -- all of which risk a Twitch account ban. That entire subsystem was removed. Note: moderator-initiated polls/predictions on another broadcaster's channel and Twitch chat-badge selection have no official API and were dropped; the broadcaster's own polls/predictions and the viewer list continue to work via Helix.
- Security: The Kick login no longer writes OAuth token responses to `--log-file`; the hidden TikTok webview is now pinned to `*.tiktok.com` (rejecting stray navigations and popups); and the auto-updater verifies the download's SHA-256 against the release manifest before running it.
- ToS: Added an in-app note where a TikTok source is added, disclosing that TikTok chat is read by scraping (may conflict with TikTok's Terms; anonymous mode recommended, a throwaway account for login). Kick now surfaces a clear message when Cloudflare is blocking its API instead of leaving a silently dead tab.
- Bugfix: The Kick "Block user" button was a silent no-op and is now hidden. Emotes no longer vanish permanently after a single transient network error -- they retry. Single-message deletions are no longer lost when "Show deletions of single messages" is off, so toggling it back on reveals them. The post-update patch-notes dialog no longer re-appears on every launch (it now tracks the notes content rather than the version number).
- Minor: YouTube-only tabs now show the channel's avatar. YouTube channel/member custom emoji are now completable in the input box. The Add-Tab dialog gives live "looks valid / unrecognized" feedback for YouTube and TikTok input as you type.
