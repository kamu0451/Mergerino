...
QColor mergedFirstMessageFallbackColor()
{
    if (mergedPlatformIndicatorShowsLineColor(
            getSettings()->mergedPlatformIndicatorMode))
    {
        return QColor(210, 92, 255, 85);
    }

    return HighlightPhrase::FALLBACK_FIRST_MESSAGE_HIGHLIGHT_COLOR;
}

bool usesFallbackColor(const QString &colorString)
{
    return !QColor(colorString).isValid();
}
...
void ColorProvider::initTypeColorMap()
{
    ...
    auto bindDynamicFallback = [this](ColorType colorType,
                                      QStringSetting &setting,
                                      auto fallbackColor) {
        auto applyFallback = [this, colorType, &setting, fallbackColor] {
            if (!usesFallbackColor(setting.getValue()))
            {
                return;
            }

            *this->typeColorMap_.at(colorType) = fallbackColor();
        };

        setting.connect([applyFallback](const auto &) { applyFallback(); },
                        false);
        getSettings()->mergedPlatformIndicatorMode.connect(
            [applyFallback](auto) { applyFallback(); }, false);
    };
    ...
    initColor(ColorType::Subscription, getSettings()->subHighlightColor,
              mergedSubscriptionFallbackColor());
    bindDynamicFallback(ColorType::Subscription,
                        getSettings()->subHighlightColor,
                        mergedSubscriptionFallbackColor);
    ...
    initColor(ColorType::FirstMessageHighlight,
              ...
              mergedFirstMessageFallbackColor());
    bindDynamicFallback(ColorType::FirstMessageHighlight,
                        getSettings()->firstMessageHighlightColor,
                        mergedFirstMessageFallbackColor);
