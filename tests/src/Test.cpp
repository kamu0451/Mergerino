...
bool isUrlAvailable(const QString &url, std::chrono::milliseconds timeout)
{
    auto parsed = QUrl(url);
    auto port = parsed.port(parsed.scheme() == "https" || parsed.scheme() == "wss"
                                ? 443
                                : 80);
    return isTcpServiceAvailable(parsed.host(), port, timeout);
}

}  // namespace

bool isTcpServiceAvailable(const QString &host, int port,
                           std::chrono::milliseconds timeout)
{
    ...
    return socket.waitForConnected(static_cast<int>(timeout.count()));
}

bool isHttpbinAvailable()
{
    return isUrlAvailable(HTTPBIN_BASE_URL, std::chrono::milliseconds(500));
}

bool isPubSubWssAvailable()
{
    return isUrlAvailable("wss://" + PUBSUB_WSS_ADDR,
                          std::chrono::milliseconds(250));
}

bool isPubSubWsAvailable()
{
    return isUrlAvailable("ws://" + PUBSUB_WS_ADDR,
                          std::chrono::milliseconds(250));
}
