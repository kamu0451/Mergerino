...
{
    if (!isPubSubWsAvailable())
    {
        GTEST_SKIP() << "PubSub WS test service is unavailable at "
                     << PUBSUB_WS_ADDR;
    }
    ...
    ASSERT_TRUE(messages[6].second.startsWith("Mergerino"));
...
{
    if (!isPubSubWssAvailable())
    {
        GTEST_SKIP() << "PubSub WSS test service is unavailable at "
                     << PUBSUB_WSS_ADDR;
    }
