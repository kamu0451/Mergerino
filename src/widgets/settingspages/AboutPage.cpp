...
AboutPage::AboutPage()
{
    ...
    auto layout = widget.setLayoutType<QVBoxLayout>();
    {
        ...
        auto projectLinks = layout.emplace<QGroupBox>("Project Links");
        {
            auto l = projectLinks.emplace<QVBoxLayout>();

            auto addOptionalLink = [&l](QStringView url, const QString &text) {
                if (url.isEmpty())
                {
                    return;
                }
                ...
                auto *label =
                    l.emplace<QLabel>(
                         QString("<a href=\"%1\">%2</a>")
                             .arg(url.toString(), text))
                        .getElement();
                label->setOpenExternalLinks(true);
            };

            addOptionalLink(LINK_MERGERINO_DOCUMENTATION,
                            "Mergerino documentation");
            addOptionalLink(LINK_MERGERINO_SOURCE, "Mergerino source code");
            addOptionalLink(LINK_MERGERINO_DISCORD, "Mergerino Discord");

            if (LINK_MERGERINO_DOCUMENTATION.isEmpty() &&
                LINK_MERGERINO_SOURCE.isEmpty() &&
                LINK_MERGERINO_DISCORD.isEmpty())
            {
                l.emplace<QLabel>(
                    "Project links are not configured in this build.");
            }

            auto *sevenTVLabel =
                l.emplace<QLabel>(
                     QString("<a href=\"%1\">7TV Discord</a>")
                         .arg(LINK_SEVENTV_DISCORD.toString()))
                    .getElement();
            sevenTVLabel->setOpenExternalLinks(true);
        }
    ...
    QObject::connect(
        b, &QLabel::linkActivated, [parent = this, name, licenseLink] {
            auto *window = new BasePopup(
                ...
                parent);
            window->setWindowTitle("Mergerino - License for " + name);
