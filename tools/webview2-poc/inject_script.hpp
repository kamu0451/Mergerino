...
namespace mergerino::poc {

// Injected before any page script runs. Monkey-patches window.WebSocket so
// any webcast-related socket's lifecycle events and every inbound message
// are forwarded to the C++ host via window.chrome.webview.postMessage.
// Binary frames are base64-encoded; text frames pass through as UTF-8.
