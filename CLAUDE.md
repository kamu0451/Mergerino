...
Mergerino is an unofficial fork of Chatterino2. It is a Qt 6, C++23 desktop chat client that merges Twitch, Kick, YouTube, and TikTok live chat into a single Chatterino-style multi-tab workflow. **Windows-only** — `CMakeLists.txt` issues a `FATAL_ERROR` on non-Windows hosts, and all CI/packaging is Windows-only. The TikTok provider hosts a hidden WebView2 (Edge runtime) per source to scrape TikTok's webcast WebSocket, so a working WebView2 runtime is a hard requirement on the user's machine. The source root namespace is still `chatterino` and the library target is `mergerino-lib`; the binary is `mergerino.exe`.
...
- **`src/providers/<service>/`** — one subdirectory per chat/emote/data provider: `twitch/` (IRC + PubSub + Helix + EventSub), `kick/` (REST + Pusher-style chat websocket + live controller), `youtube/` (`YouTubeLiveChat` polls the InnerTube `get_live_chat` endpoint), `tiktok/` (`TikTokLiveChat` hosts a hidden WebView2 that runs TikTok's own webapp and forwards webcast WebSocket frames to C++), plus emote providers `seventv/`, `bttv/`, `ffz/`, and the aggregator in `merged/`. `YouTubeLiveChat` and `TikTokLiveChat` are accessed via static `getOrCreateShared(source)` registries (strong refs, process-lifetime), so multiple `MergedChannel`s referencing the same source share one instance.
- **`src/providers/merged/MergedChannel`** — the Mergerino-specific concept. A `MergedChannel` owns child `ChannelPtr`s for Twitch and Kick and shared-pointer references to a `YouTubeLiveChat` and `TikTokLiveChat`, subscribes to each source's `messageAppended`/`messageReplaced` signals, and rebroadcasts a platform-tagged copy on the merged channel. Keys messages via `messageKey()` to de-duplicate edits across sources. When extending merged behavior, preserve the per-platform `SignalHolder`s and the `mirroredMessages_` map — lifetime and replacement rely on them. `MergedChannel::reconnect()` is a deliberate lightweight rebind (called from `Split::setChannel` during layout-restore); user-driven reconnects flow through `MergedChannel::userReconnect()`, which forwards to Twitch/Kick but skips the shared YouTube/TikTok providers.
